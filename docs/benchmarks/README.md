...
- `native-distance-kernels.json` / `native-distance-kernels.md`: optional
  local artifacts generated by the MVP2 native kernel benchmark wrapper.
- `../assets/native-distance-kernels.svg`: optional local chart generated from
  the native kernel benchmark. These files are ignored until a stable native
  benchmark artifact is ready to publish.
